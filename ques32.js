...
var new_users = ["Ahmed", "Manzoor", "Hadad", "Ayaz"];
new_users.forEach(function (new_users) {
    var new_userLower = new_users.toLowerCase();
    var usernameTaken = current_users.some(function (current_users) { return current_users.toLowerCase() === new_userLower; });
    if (usernameTaken) {
        console.log("".concat(new_users, " Needs to choose a new username becaus it's already taken"));
    }
    else {
